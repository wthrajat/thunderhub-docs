...
This guide covers connecting a self-hosted ThunderHub instance to a [Voltage](https://voltage.cloud) Lightning node. Voltage nodes run litd under the hood, so ThunderHub can connect using the `litd` account type to get full node management including Taproot Assets support.
...
1. Log into [app.voltage.cloud](https://app.voltage.cloud/login) and click **Create Node** → **Lightning Node**.
...
<Callout type="info">
  ...
  testing Taproot Assets before moving to mainnet.
</Callout>
...
Here is a screenshot of where to find the server URL on the Voltage dashboard:
...
Here is a screenshot of the Macaroon Bakery on the Voltage dashboard:
