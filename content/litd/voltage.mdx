...
- ThunderHub [v0.15.5](https://github.com/apotdevin/thunderhub/releases/tag/v0.15.5) or later (the `latest` Docker tag works)
- [Docker](https://docs.docker.com/get-docker/) installed (for running ThunderHub using Docker)
  <Callout type="info">
    You can also run ThunderHub using Node.js if you prefer, but Docker is
...
5. Once the node is running, enable Taproot Assets in the node settings. See the screenshot below for the toggle location.
   ![Enable Taproot Assets](/assets/voltage-ta.png)
6. Configure a **price oracle** so your node can trade Taproot Assets. In **Node Settings**, set the price oracle URL to:
   - **Mainnet:** `rfqrpc://price-oracle.amboss.tech:443`
   - **Mutinynet:** `rfqrpc://price-oracle-dev.amboss.tech:443`

   See the [Price Oracle](/litd/price-oracle) guide for more details and alternative configurations.
...
<Callout type="warning">
  You need a **superadmin** macaroon (not just admin) to access all
  functionality offered by litd.
</Callout>
